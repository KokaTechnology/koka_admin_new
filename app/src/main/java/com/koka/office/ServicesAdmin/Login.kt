...
class Login : Fragment() {
    ...
    private fun loginUser() {
        ...
        if (emails.isEmpty() || password.isEmpty()) {
            Toast.makeText(
                requireContext(),
                "Can Not Log in,Please fillup all the fields",
                Toast.LENGTH_LONG
            ).show()
            return
        } else {
            if (emails == "adminkoka@gmail.com" && password == "123456") {
                mAuth.signInWithEmailAndPassword(emails, password).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(
                            requireContext(),
                            "Something Wrong!  Try again after few seconds, Check Internet Connection",
                            Toast.LENGTH_LONG
                        ).show()
                ...
                Toast.makeText(
                    requireContext(),
                    "Admin Email or Password is Wrong!!",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
